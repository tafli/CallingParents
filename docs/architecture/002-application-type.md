...
- **Internationalization (i18n)**: the PWA supports multiple languages (German and English) via a lightweight custom i18n module (`i18n.js`). Language detection follows a priority chain: localStorage override → `navigator.language` → German fallback. All user-facing strings in both static HTML (via `data-i18n` attributes) and dynamic JS (via a `t(key, params)` function) are translatable. A language picker in the settings view allows switching at runtime; the choice persists in localStorage. Adding a new language requires only adding a translation object to `i18n.js` and a picker entry to `availableLanguages`.
