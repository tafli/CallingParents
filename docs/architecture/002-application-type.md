...
- **Disconnected state UX**: when ProPresenter is unreachable, the PWA provides clear visual feedback through four mechanisms: (1) the Send button is disabled so users cannot attempt futile sends, (2) a warning banner appears below the header, (3) the children name grid is dimmed and non-interactive, and (4) the header background changes from blue to orange. All four indicators revert automatically when connectivity is restored. The theme-color meta tag also updates so the browser chrome reflects the connection state.
- **Toast notifications**: brief feedback messages appear for send/clear results and errors, auto-dismissing after 3 seconds.
- **Haptic feedback**: on successful send, the PWA calls `navigator.vibrate(100)` (100 ms pulse) for tactile confirmation. The call is guarded so browsers without vibration support are unaffected.
- **Fixed input area layout**: the main view uses a flex column layout (`height: 100vh`, `overflow: hidden` on body). The children grid scrolls independently within its own area while the input field and Send button remain **always visible** at the bottom of the screen — no scrolling required to reach the action area, regardless of how many names are in the grid.
- **Internationalization (i18n)**: the PWA supports multiple languages (German and English) via a lightweight custom i18n module (`js/i18n.js`). Translations are stored as separate JSON files in `lang/` (e.g. `lang/de.json`, `lang/en.json`) and loaded on demand. Language detection follows a priority chain: localStorage override → `navigator.language` → German fallback. All user-facing strings in both static HTML (via `data-i18n` attributes) and dynamic JS (via a `t(key, params)` function) are translatable. A language picker in the settings view allows switching at runtime; the choice persists in localStorage. Adding a new language requires only adding a `lang/{code}.json` file and a picker entry to `availableLanguages` in `i18n.js`.
